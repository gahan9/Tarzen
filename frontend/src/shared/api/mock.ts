...
/**
 * Local mock of `POST /api/footprint` for UI development and tests, gated by
 * `VITE_USE_MOCK_API`. It mirrors the backend contract (rule-based numbers are
 * illustrative only) so the UI can be exercised without a live backend. A
 * later Phase 5 worker owns real end-to-end verification.
 */
...
export function mockFootprint(request: FootprintRequest): FootprintResponse {
  ...
  return {
    kg_co2e: rounded,
    breakdown: [
      { label: `${request.mode} travel`, kg_co2e: rounded },
    ],
    insight: {
      ...
      actions: needsContext
        ...
        : ["Try combining errands into one trip", "Consider rail for trips under 700 km"],
